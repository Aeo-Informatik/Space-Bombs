...
public class X3 extends Bomb
{
    ...
    private int X;
    private int explosionRepeat;
    
    //Sound
    private long soundId = -1;
    private Sound fuseSound;
    
    public X3(ThinGridCoordinates pos, int playerId,  int range, int X, int explosionRepeat, MapLoader map, EntityManager entityManager) 
    {
        ...
        super(pos, playerId, range, Constants.X3BOMBEXPLOSIONTIME, Constants.X3BOMBEXPLOSIONDURATION, Constants.X3BOMBDELAYEXPLODEAFTERHITBYBOMB, map, entityManager);
        super.setBombAnimation(TextureManager.x3Anim);
        this.X = X;
        this.explosionRepeat = explosionRepeat;
    }
    ...
    public void render()
    {
        ...
        if(!map.isCellBlocked(new MapCellCoordinates(pos.getX(), pos.getY())) && !isExploded)
        {
            ...
            {
                player.playSoundInDistance(fuseSound, soundId, pos, Constants.X3FUSEMOD);
            }
            ...
            if(timerTillExplosion >= explosionTime)
            {
                ...
                if(timerTillExplosionDelete >= explosionDuration)
                {
                    ...
                    if(X <= explosionRepeat && !map.isBombPlaced(new MapCellCoordinates(pos.getX(), pos.getY())))
                    {   
                        entityManager.getBombManager().spawnBomb(new X3(pos, playerId, super.getRange(), X, explosionRepeat, map, entityManager));
                    }
                }else
                {
