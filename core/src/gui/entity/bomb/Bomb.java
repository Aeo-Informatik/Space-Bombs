...
public abstract class Bomb extends Entity
{
    ...
    //Constructor
    public Bomb(ThinGridCoordinates pos, int playerId, int range, float explosionTime, float explosionDuration,float delayExplodeAfterHitByBomb, MapLoader map, EntityManager entityManager)
    { 
        super(pos, map, entityManager);
        ...
        {
            case 0:
                Constants.BOMBIDCOUNTERP0 += 1;
                bombId = Constants.BOMBIDCOUNTERP1;
                
                if(bombId > 5000)
                    Constants.BOMBIDCOUNTERP0 = 4000;
                break;
    ...
    public void dropFromBlock (MapCellCoordinates localCellPos)
    {
        ...
        {
            entityManager.getItemManager().spawnCoin(localCellPos, Constants.COINVALUE);
